...
Material::Material(std::vector<sf::Color> color, float constant_force, int randomSpawn, void (*update)(ParticleContainer* container, int x, int y, bool even))
...
void waterUpdate(ParticleContainer* container, int x, int y, bool even) {
    ...
    if(self->updated != even) {
        for(int i = 0; i < self->speed_y; i++) {
            ...
            container->getParticle(x, y + 1).getMaterial() != Materials::americaWantsIt) {
                self->speed_y = 0;
            } else {
        ...
        container->getParticle(x, y + 1).getMaterial() != Materials::americaWantsIt &&
        container->getParticle(x, y + 1).getMaterial() != Materials::smoke) {
            if((container->getParticle(x + 1, y).getMaterial() == Materials::air || container->getParticle(x + 1, y).getMaterial() == Materials::americaWantsIt || container->getParticle(x + 1, y).getMaterial() == Materials::smoke) &&
            (container->getParticle(x - 1, y).getMaterial() == Materials::air || container->getParticle(x - 1, y).getMaterial() == Materials::americaWantsIt || container->getParticle(x - 1, y).getMaterial() == Materials::smoke))
                self->speed_x = (rand() % 2 * 2 - 1) * 5;
            else if(container->getParticle(x + 1, y).getMaterial() == Materials::air || container->getParticle(x + 1, y).getMaterial() == Materials::smoke || container->getParticle(x + 1, y).getMaterial() == Materials::americaWantsIt)
                self->speed_x = 5;
            else if(container->getParticle(x - 1, y).getMaterial() == Materials::air || container->getParticle(x + 1, y).getMaterial() == Materials::smoke || container->getParticle(x + 1, y).getMaterial() == Materials::americaWantsIt)
                self->speed_x = -1;
            else
            ...
            if(self->speed_x > 0) {
                for(int i = 0; i < self->speed_x; i++) {
                    if(container->getParticle(x + 1, y).getMaterial() != Materials::air && container->getParticle(x + 1, y).getMaterial() != Materials::smoke && container->getParticle(x + 1, y).getMaterial() != Materials::americaWantsIt)
                        break;
...
void initMaterials() {
    Materials::air = new Material({{90, 90, 90}}, 0, 0);
    Materials::sand = new Material({{245, 210, 92}, {237, 205, 88}, {239, 207, 90}, {235, 203, 86}, {230, 200, 84}}, 0.08, 80, &sandUpdate);
    Materials::water = new Material({{52, 145, 173}, {53, 147, 176}, {47, 140, 169}}, 0.08, 80, &waterUpdate);
    Materials::wood = new Material({{155, 116, 54}, {154, 115, 53}, {156, 117, 55}, {150, 111, 51}}, 0, 0);
    Materials::fire = new Material({{222, 70, 12}, {222, 120, 16}, {222, 125, 16}, {222, 127, 16}, {222, 130, 16}, {222, 182, 16}}, 0, 20, &fireUpdate);
    Materials::stone = new Material({{133, 133, 133}, {135, 135, 135}, {131, 131, 131}}, 0, 0);
    Materials::smoke = new Material({{65, 65, 59}, {60, 60, 54}, {70, 70, 64}}, 0.08, 40, &smokeUpdate);
    Materials::gunpowder = new Material({{36, 36, 36}}, 0.08, 40, &sandUpdate);
    Materials::acid = new Material({{31, 255, 61}, {32, 255, 62}, {31, 255, 61}}, 0.08, 40, &acidUpdate);
    Materials::americaWantsIt = new Material({{10, 10, 10}, {12, 12, 12}, {9, 9, 9}}, 0.08, 40, &americaWantsItUpdate);
    Materials::lava = new Material({{252, 91, 16}, {252, 95, 16}, {252, 97, 16}, {252, 102, 16}, {252, 142, 16}}, 0.08, 40, &lavaUpdate);
}
